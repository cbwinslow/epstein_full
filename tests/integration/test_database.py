...
class TestDatabaseOperations:
    ...
    def test_queue_workflow(self, storage_manager):
        ...
        # Get queued items
        items = storage_manager.get_queued_items(media_type="news", status="pending", limit=10)
    ...
    def test_news_article_storage(self, storage_manager, sample_news_article):
        ...
        # Retrieve article
        articles = storage_manager.get_news_articles(source_domain="example.com", limit=1)
    ...
    def test_video_metadata_storage(self, storage_manager):
        ...
        # Retrieve
        videos = storage_manager.get_videos(platform="youtube", limit=1)
...
class TestEndToEndWorkflow:
    ...
    async def test_full_discovery_and_collection(
        self, news_discovery_agent, news_collector, storage_manager, mocker
    ):
        """
        ...
        mocker.patch(
            "media_acquisition.agents.discovery.news.requests.get", return_value=mock_response
        )
        ...
        for article in discovered:
            queue_id = storage_manager.add_to_queue(
                ...
                metadata={"title": article.title, "source_domain": article.source_domain},
            )
