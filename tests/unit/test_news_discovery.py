...
class TestNewsDiscoveryAgent:
    ...
    async def test_gdelt_empty_response(self, mock_get, agent_config: AgentConfig):
        ...
        # Execute
        agent = NewsDiscoveryAgent(agent_config)
        results = await agent.search(keywords=["NonExistentKeyword12345"], max_results=10)
    ...
    async def test_wayback_search(self, mock_get, agent_config: AgentConfig):
        ...
        # Execute
        agent = NewsDiscoveryAgent(agent_config)
        results = await agent._search_wayback(url="https://example.com/article", date="2024-01-01")
    ...
    @pytest.mark.asyncio
    @patch("media_acquisition.agents.discovery.news.requests.get")
    async def test_article_structure_validation(
        self, mock_get, agent_config: AgentConfig, mock_gdelt_response
    ):
