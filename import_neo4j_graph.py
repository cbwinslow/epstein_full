...
async def import_persons(conn: asyncpg.Connection) -> int:
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            aliases = (
                row.get("aliases:string[]", "").split(";") if row.get("aliases:string[]") else []
            )
            occupations = (
                row.get("occupations:string[]", "").split(";")
                if row.get("occupations:string[]")
                else []
            )
            sources = (
                row.get("sources:string[]", "").split(";") if row.get("sources:string[]") else []
            )
...
async def import_other_nodes(conn: asyncpg.Connection) -> int:
    ...
    for node_type, filename in [
        ("Event", "event_nodes.csv"),
        ("LegalCase", "legal_case_nodes.csv"),
    ]:
...
async def verify_import(conn: asyncpg.Connection):
    ...
    for row in sample:
        print(
            f"  {row['start_name']} --[{row['rel_type']}]--> {row['end_name']} (conf: {row['confidence']})"
        )
