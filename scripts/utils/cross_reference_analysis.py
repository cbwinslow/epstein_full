...
def generate_summary_report(fbi_stats, house_stats, index_stats):
    ...
    print(
        f"   - {fbi_stats['overlap_count']:,} overlap with DOJ ({fbi_stats['overlap_count'] / fbi_stats['fbi_unique_docs'] * 100:.1f}%)"
    )
    ...
    print(
        f"   - {index_stats['covered_docs']:,}/{index_stats['total_docs']:,} documents covered ({index_stats['covered_docs'] / index_stats['total_docs'] * 100:.1f}%)"
    )

    print("\n💡 Value Added:")
    print(
        f"   - FBI embeddings provide {fbi_stats['fbi_exclusive']:,} new documents not in DOJ EFTA"
    )
    print("   - House Oversight adds document-level embeddings vs thread-level emails we had")
    print(
        f"   - Full Epstein Index provides text extracts for {index_stats['covered_docs']:,} documents"
    )
