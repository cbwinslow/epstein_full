...
# URLs
HOUSE_SEARCH_URL = (
    "https://disclosures-clerk.house.gov/PublicDisclosure/FinancialDisclosure/ViewSearch"
)
...
class HouseDisclosureScraper:
    ...
    def search_members(
        self, last_name: str = None, state: str = None, year: int = None
    ) -> List[Dict]:
        """
                ...
                for row in rows:
                    ...
                    if len(cells) >= 4:
                        ...
                        if link_tag and link_tag.get("href"):
                            result["link"] = (
                                f"https://disclosures-clerk.house.gov{link_tag['href']}"
                            )
    ...
    def get_public_disclosures(self) -> List[Dict]:
        ...
        try:
            ...
            for link in soup.find_all("a", href=True):
                ...
                if any(x in href.lower() for x in ["disclosure", "financial", "report"]) and any(
                    x in href.lower() for x in [".pdf", ".txt"]
                ):
                    results.append(
                        {
                            "text": text,
                            ...
                            else f"https://www.disclosure.senate.gov{href}",
                            "source": "senate",
                        }
                    )
    ...
    def search_by_name(self, name: str) -> List[Dict]:
        ...
        try:
            ...
            for item in soup.find_all(["div", "tr"], class_=lambda x: x and "result" in x.lower()):
                result = {"name": name, "text": item.get_text(strip=True), "link": None}
        ...
        try:
            ...
            for link in soup.find_all("a", href=True):
                ...
                if "periodic" in text or "transaction" in text or "ptr" in text:
                    ptrs.append({"text": link.get_text(strip=True), "url": link["href"]})
...
def main():
    ...
    parser.add_argument(
        "--transactions", action="store_true", help="Get periodic transaction reports"
    )
    ...
    if args.house or (not args.house and not args.senate and not args.transactions):
        ...
        results = scraper.search_members(last_name=args.name, state=args.state, year=args.year)
    ...
    if args.transactions:
        ...
        results = {"house": house_trans, "senate": senate_trans}
