...
class MigrationRunner:
    ...
    def parse_migration_file(self, file_path: Path) -> dict:
        ...
        return {"version": version, "name": name, "content": content, "file": file_path}
    ...
    def show_status(self):
        ...
        with self.conn.cursor() as cur:
            ...
            for row in cur.fetchall():
                print(
                    f"  ✓ {row[0]}: {row[1]} (applied: {row[2].strftime('%Y-%m-%d %H:%M')}, {row[3]}ms)"
                )
...
Examples:
  ...
  %(prog)s --verify             # Verify schema integrity
    ...
    parser.add_argument(
        "--database-url", default=DEFAULT_DATABASE_URL, help="PostgreSQL connection string"
    )

    parser.add_argument("--status", action="store_true", help="Show migration status")

    parser.add_argument("--version", help="Apply specific migration version")
