...
# Configuration
DB_CONFIG = {"host": "localhost", "port": "5432", "user": "cbwinslow", "password": "123qweasd"}

ADMIN_CONFIG = {"host": "localhost", "port": "5432", "user": "postgres"}
...
"""
    ...
    reload_result = subprocess.run(
        ["sudo", "systemctl", "reload", "postgresql"], capture_output=True, text=True
    )

    if reload_result.returncode != 0:
        # Try alternative reload method
        subprocess.run(
            ["sudo", "-u", "postgres", "pg_ctl", "reload"], capture_output=True, text=True
        )
