...
class DataQualityValidator:
    ...
    def run_sql_validation(self) -> Dict:
        ...
        results = {"orphaned_records": [], "duplicates": [], "completeness": [], "table_stats": {}}
        ...
        try:
            with self.conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                for row in rows:
                    print(
                        f"  [{row['severity']}] {row['check_type']}: {row['status']} - {row['details']}"
                    )
        ...
        try:
            with self.conn.cursor() as cur:
                ...
                if stats["total_documents"] > 0:
                    print(
                        f"  With OCR: {stats['with_ocr']:,} ({stats['with_ocr'] * 100 / stats['total_documents']:.1f}%)"
                    )
                    print(
                        f"  With classification: {stats['with_classification']:,} ({stats['with_classification'] * 100 / stats['total_documents']:.1f}%)"
                    )
                    print(
                        f"  With entities: {stats['with_entities']:,} ({stats['with_entities'] * 100 / stats['total_documents']:.1f}%)"
                    )
                    print(
                        f"  With redactions: {stats['with_redactions']:,} ({stats['with_redactions'] * 100 / stats['total_documents']:.1f}%)"
                    )
                else:
                ...
                if unprocessed > 0:
                    ...
                    print("  ✓ All documents have some processing data")
    ...
    def validate_sqlite_databases(self) -> List[Dict]:
        ...
        for db_path in sqlite_dbs:
            if os.path.exists(db_path):
                try:
                    ...
                    results.append({"file": db_path, "status": "valid", "tables": len(tables)})
                except Exception as e:
                    print(f"  ✗ {os.path.basename(db_path)}: {e}")
                    results.append({"file": db_path, "status": "error", "error": str(e)})
                    self.issues.append(
                        {
                            "type": "sqlite_error",
                            "severity": "HIGH",
        ...
        for gz_path in gz_dbs:
            if os.path.exists(gz_path):
                ...
                results.append(
                    {"file": gz_path, "status": "compressed", "size_mb": size / 1024 / 1024}
                )
...
"""
        ...
        for issue in report["issues"][:10]:  # Top 10 issues
            prompt += (
                f"- [{issue['severity']}] {issue['type']} in {issue['table']}: {issue['details']}\n"
            )
...
"""
    ...
    def run_full_validation(self, ai_analysis: bool = False) -> Dict:
        ...
        if ai_analysis:
            ...
            ai_prompt_path = os.path.join(
                REPORT_DIR, f"ai_analysis_prompt_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
            )
...
def main():
    parser = argparse.ArgumentParser(description="Epstein Project Data Quality Validator")
    parser.add_argument("--quick", action="store_true", help="Run quick validation only")
    parser.add_argument("--sql-only", action="store_true", help="Run SQL validation only")
    parser.add_argument("--ai-report", action="store_true", help="Generate AI analysis prompt")
    parser.add_argument(
        "--apply-views", action="store_true", help="Apply data quality SQL views first"
    )
