...
def get_document_cooccurrences(conn, min_cooccurrence=2):
    ...
    print(
        f"Found {len(connections):,} document connections (min {min_cooccurrence} co-occurrences)"
    )
    return connections
...
def verify_connections(conn):
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        for row in cur.fetchall():
            print(
                f"  {row['source_entity'][:40]}: {row['connections']} connections, weight={row['total_weight']}"
            )
...
def main():
    ...
    parser.add_argument(
        "--source",
        choices=["emails", "flights", "documents", "all"],
        default="all",
