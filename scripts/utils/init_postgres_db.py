...
def check_duplicates(conn, table, key_field, values):
    ...
    for i in range(0, len(values), batch_size):
        ...
        cursor.execute(
            f"SELECT {key_field} FROM {table} WHERE {key_field} IN ({placeholders})", batch
        )
...
def import_full_text_corpus():
    ...
    print("Importing from full_text_corpus.db...")
    ...
    try:
        ...
        sqlite_cursor.execute(
            "SELECT efta_id, filename, dataset, filepath, file_size, page_count, document_type, classification, metadata FROM documents"
        )
        ...
        if documents:
            ...
            if new_documents:
                ...
                pg_conn.commit()
                print(
                    f"    ✓ Imported {len(new_documents)} new documents (skipped {len(existing)} duplicates)"
                )
            else:
        ...
        if pages:
            ...
            for i in range(0, len(pages), batch_size):
                ...
                pg_cursor.execute(
                    "SELECT id, efta_id FROM documents WHERE efta_id IN %s", (tuple(efta_ids),)
                )
...
def import_knowledge_graph():
    ...
    print("Importing from knowledge_graph.db...")
    ...
    try:
        ...
        sqlite_cursor.execute(
            "SELECT name, entity_type, normalized_name, category, confidence, metadata FROM entities"
        )
        ...
        for row in sqlite_cursor.fetchall():
            ...
            entities.append(
                (name, ent_type, norm_name, category, confidence, json.dumps(metadata or {}))
            )
        ...
        sqlite_cursor.execute(
            "SELECT source_name, target_name, relationship_type, confidence FROM relationships"
        )
...
def import_communications():
    ...
    print("Importing from communications.db...")
    ...
    try:
        ...
        # Import emails
        sqlite_cursor.execute(
            "SELECT communication_type, sent_date, subject, content, sender_email FROM emails"
        )
...
def import_transcripts():
    ...
    print("Importing from transcripts.db...")
...
def import_ocr_results():
    ...
    print("Importing from ocr_database.db...")
