...
class CrossReferenceValidator:
    ...
    def check_sqlite_databases(self):
        ...
        db_checks = {
            ...
            "redaction_analysis_v2.db": {"redactions": "SELECT COUNT(*) FROM redactions"},
            "communications.db": {"emails": "SELECT COUNT(*) FROM emails"},
            "transcripts.db": {"transcripts": "SELECT COUNT(*) FROM transcripts"},
            "image_analysis.db": {"images": "SELECT COUNT(*) FROM images"},
            "knowledge_graph.db": {"entities": "SELECT COUNT(*) FROM entities"},
        }
        ...
        for db_name, queries in db_checks.items():
            ...
            try:
                ...
                self.results["database_comparison"][db_name] = {"status": "error", "error": str(e)}
    ...
    def check_postgresql(self):
        ...
        try:
            ...
            pg_results = {"status": "connected", "tables": {}}
            ...
            self.results["database_comparison"]["postgresql"] = {"status": "error", "error": str(e)}
    ...
    def validate_document_integrity(self):
        ...
        total_raw = sum(ds["actual"] for ds in self.results["dataset_comparison"].values())
        ...
        if os.path.exists(db_path):
            try:
                ...
                self.results["document_integrity"] = {"error": str(e)}
    ...
    def generate_recommendations(self):
        ...
        incomplete_datasets = [
            name
            for name, data in self.results["dataset_comparison"].items()
        ...
        missing_dbs = [
            name
            for name, data in self.results["database_comparison"].items()
    ...
    def generate_report(self):
        ...
        for name, data in sorted(self.results["dataset_comparison"].items()):
            ...
            print(
                f"{status} {name:12} : {actual:>8,} / {expected:>8,} ({percent:>5.1f}%) - {data.get('description', '')}"
            )
        ...
        if self.results["missing_data"]:
            ...
            print("-" * 80)
            for item in sorted(
                self.results["missing_data"], key=lambda x: x["severity"], reverse=True
            ):
                symbol = "🔴" if item["severity"] == "high" else "🟡"
                print(
                    f"{symbol} {item['dataset']}: {item['missing']:,} files missing ({item['type']})"
                )
        ...
        if not self.results["recommendations"]:
            ...
            for i, rec in enumerate(
                sorted(self.results["recommendations"], key=lambda x: x["priority"]), 1
            ):
    ...
    def save_results(self):
        ...
        output_file = (
            f"{DATA_ROOT}/validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
if __name__ == "__main__":
    ...
    critical_issues = [m for m in results.get("missing_data", []) if m["severity"] == "high"]
