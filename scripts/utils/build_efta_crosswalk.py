...
class EFTACrosswalkBuilder:
    ...
    def scan_postgresql(self, batch_size: int = 10000) -> int:
        ...
        while True:
            with self.conn.cursor() as cur:
                ...
                data = [(efta, True, dataset, ingested) for efta, dataset, ingested in rows]
    ...
    def update_data_quality_flags(self):
        ...
        print("✓ Data quality flags updated:")
...
def main():
    ...
    parser.add_argument("--scan-postgresql", action="store_true", help="Scan PostgreSQL documents")
    parser.add_argument("--scan-raw-files", action="store_true", help="Scan raw-files directory")
    parser.add_argument("--scan-hf-datasets", action="store_true", help="Scan HF datasets")
    parser.add_argument("--full-sync", action="store_true", help="Sync all sources (full rebuild)")
    parser.add_argument("--analyze", action="store_true", help="Show coverage analysis")
    ...
    if not any(
        [
            args.scan_postgresql,
            args.scan_raw_files,
            args.scan_hf_datasets,
            args.full_sync,
            args.analyze,
            args.validate,
        ]
