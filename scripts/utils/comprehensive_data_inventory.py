...
class DataInventory:
    ...
    def scan_databases(self):
        ...
        # SQLite databases
        db_paths = [f"{RESEARCH_DATA}", f"{DATA_ROOT}/databases", f"{PROJECT_ROOT}/databases"]
        ...
        for db_path in db_paths:
            ...
            for db_file in Path(db_path).rglob("*.db"):
                try:
                    ...
                    self.inventory["databases"][str(db_file)] = {"error": str(e)}
        ...
        try:
            ...
            self.inventory["databases"]["postgresql_epstein"] = {"error": str(e)}
    ...
    def scan_repositories(self):
        ...
        for name, path in repos:
            if os.path.exists(path):
                ...
                self.inventory["repositories"][name] = {"path": path, "exists": False}
    ...
    def run(self):
        ...
        # Save to file
        output_path = (
            f"{PROJECT_ROOT}/data_inventory_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
