...
async def main():
    ...
    hf_sources = [
        ...
        (
            "HF Data Text",
            "huggingface",
            "hf-new-datasets/epstein-data-text",
    ...
    icij_sources = [
        (
            "ICIJ Entities",
            "icij",
            "icij_extracted/nodes-entities.csv",
            "icij_entities",
            814617,
            "pending",
            1,
        ...
        (
            "ICIJ Officers",
            "icij",
            "icij_extracted/nodes-officers.csv",
            "icij_officers",
            1800000,
            "pending",
            1,
        ...
        (
            "ICIJ Addresses",
            "icij",
            "icij_extracted/nodes-addresses.csv",
            "icij_addresses",
            700000,
            "pending",
            1,
        ...
        (
            "ICIJ Intermediaries",
            "icij",
            "icij_extracted/nodes-intermediaries.csv",
            "icij_intermediaries",
            38000,
            "pending",
            1,
        ...
        (
            "ICIJ Others",
            "icij",
            "icij_extracted/nodes-others.csv",
            "icij_others",
            4000,
            "pending",
            1,
        ...
        (
            "ICIJ Relationships",
            "icij",
            "icij_extracted/relationships.csv",
            "icij_relationships",
            3339272,
            "pending",
            1,
    ...
    for name, stype, path, table, expected, status, priority in hf_sources + icij_sources:
        ...
        """,
            name,
            stype,
            str(Path.home() / "workspace/epstein-data" / path),
            table,
            expected,
            status,
            priority,
    ...
    if icij_dir.exists():
        for csv_file in icij_dir.glob("*.csv"):
            ...
            source_id = await conn.fetchval(
                "SELECT id FROM data_sources WHERE source_name = $1", source_name
            )
            if source_id:
                ...
                file_id = await conn.fetchval(
                    "SELECT id FROM data_files WHERE full_path = $1", str(csv_file)
                )
                if file_id:
                    await conn.execute(
                        """
