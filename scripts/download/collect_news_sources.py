...
class NewsAPICollector:
                        ...
                        articles.append(
                            {
                                ...
                                "summary": summary[:500],
                                "keywords_matched": [
                                    kw for kw in keywords if kw.lower() in content_text
                                ],
                            }
                        )
    ...
    async def fetch_sitemap(
        self, source: str, year: int, month: int, keywords: List[str]
    ) -> List[Dict]:
        ...
        try:
            async with self.session.get(url, timeout=30, ssl=False) as response:
                ...
                for url_elem in root.findall("ns:url", ns):
                    ...
                    # Check keywords
                    if any(
                        kw.lower() in article_url.lower() or kw.lower() in title.lower()
                        for kw in keywords
                    ):
                        urls.append(
                            {
    ...
    async def collect_by_keywords(
        self, keywords: List[str], year_start: int, year_end: int
    ) -> List[Dict]:
...
class GoogleNewsCollector:
    ...
    async def search_google_news(self, query: str, year_start: int, year_end: int) -> List[Dict]:
...
async def run_collection(keywords: List[str], year_start: int, year_end: int, output_file: str):
    """Run full collection."""
    ...
    for keyword in keywords[:3]:  # Limit to avoid rate limits
        articles = await google_collector.search_google_news(
            query=keyword, year_start=year_start, year_end=year_end
        )
        all_articles.extend(articles)
