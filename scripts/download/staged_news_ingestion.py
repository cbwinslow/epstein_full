...
from run_news_ingestion import complete_ingestion_run, create_ingestion_run, update_run_results
...
class StagedIngestionOrchestrator:
    ...
    async def run_discovery(
        self, stage_run_id: str, start_date: str, end_date: str, max_results: int
    ) -> List[NewsArticleURL]:
        ...
        try:
            ...
            date_range_dt = (dt.strptime(start_date, "%Y-%m-%d"), dt.strptime(end_date, "%Y-%m-%d"))
        ...
        await update_run_results(stage_run_id, len(all_articles), 0, 0, 0, source_breakdown)

        logger.info(f"[STAGE {stage_run_id[:8]}] Total discovered: {len(all_articles)}")
        return all_articles

    async def run_stage(self, start_date: str, end_date: str, max_results: int) -> Dict:
        ...
        try:
            # Discovery
            articles = await self.run_discovery(stage_run_id, start_date, end_date, max_results)
...
def main():
    ...
    parser.add_argument(
        "--articles-per-year", type=int, default=500, help="Target articles per year (default: 500)"
    )
    parser.add_argument(
        "--batch-months", type=int, default=3, help="Process in X-month batches (default: 3)"
    )
    parser.add_argument(
        "--keywords",
        nargs="+",
