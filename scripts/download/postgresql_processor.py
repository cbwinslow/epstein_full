...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            "/home/cbwinslow/workspace/epstein/processed/postgresql_processing.log"
        ),
...
def process_pdf_full_pages(pdf_path, nlp, max_pages=None):
    ...
    try:
        ...
        logger.info(
            f"Processing {pdf_path}: {total_pages} total pages, processing {pages_to_process}"
        )
...
def process_single_file_postgresql(filename, input_dir, nlp, max_pages_per_doc=None):
    ...
    text, char_count, page_count, entities = process_pdf_full_pages(
        file_path, nlp, max_pages_per_doc
    )
        ...
        if conn:
            try:
                with conn.cursor() as cur:
                    ...
                    logger.info(
                        f"{filename}: {char_count:,} chars, {len(entities)} entities ({processing_time:.2f}s)"
                    )
                    return {
                        "filename": filename,
...
def process_dataset_postgresql(
    input_dir, workers=8, max_files=None, max_pages_per_doc=None, dataset_name=None
):
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = [
            executor.submit(
                process_single_file_postgresql, filename, input_dir, nlp, max_pages_per_doc
            )
        ...
        for i, future in enumerate(as_completed(futures)):
            ...
            if result:
                ...
                logger.info(
                    f"{i + 1}/{len(pdf_files)} {result['filename']}: {result['char_count']:,} chars, {result['entities']} entities ({result['processing_time']:.2f}s)"
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process Epstein PDF files and store in PostgreSQL"
    )
