        ...
        for name, url in endpoints.items():
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Save JSON
                    output_file = (
                        DATA_DIR / f"quiver_{name}_{datetime.now().strftime('%Y%m%d')}.json"
                    )
                    with open(output_file, "w") as f:
                        json.dump(data, f, indent=2)

                    logger.info(
                        f"Saved {len(data) if isinstance(data, list) else 'N/A'} records to {output_file}"
                    )
    ...
    def download_propublica_financial(self, api_key: str) -> Path:
        ...
        try:
            ...
            response = self.session.get(
                f"{base_url}/118/senate/members.json", headers=headers, timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                output_file = (
                    DATA_DIR / f"propublica_senate_118_{datetime.now().strftime('%Y%m%d')}.json"
                )
                with open(output_file, "w") as f:
                    json.dump(data, f, indent=2)
...
def main():
    ...
    parser.add_argument(
        "--year", type=int, default=datetime.now().year, help="Year for disclosures"
    )
