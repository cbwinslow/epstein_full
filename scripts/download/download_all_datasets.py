...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"{LOG_DIR}/download_all_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
...
DOWNLOADS = [
    # ICIJ datasets
    (
        "https://offshoreleaks-data.icij.org/offshore_leaks_csv.2022-11-15.zip",
        "icij/offshore-leaks.zip",
    ),
    (
        "https://offshoreleaks-data.icij.org/panama_papers_csv.2022-05-01.zip",
        "icij/panama-papers.zip",
    ),
    (
        "https://offshoreleaks-data.icij.org/paradise_papers_csv.2017-11-05.zip",
        "icij/paradise-papers.zip",
    ),
    # HuggingFace datasets (using wget for raw files)
    (
        "https://huggingface.co/datasets/genevera/FULL_EPSTEIN_INDEX/resolve/main/data/train-00000-of-00001.parquet",
        "hf/full-epstein-index.parquet",
    ),
]
