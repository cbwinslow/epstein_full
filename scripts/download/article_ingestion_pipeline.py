...
class ArticleMetadataExtractor:
    ...
    def _assess_credibility(
        self, source_type: str, has_author: bool, has_date: bool
    ) -> Dict[str, Any]:
    ...
    async def _store_article(self, metadata: Dict) -> Optional[int]:
        ...
        try:
            with conn.cursor() as cur:
                ...
                logger.info(
                    f"✓ Stored article ID {article_id}: {metadata.get('title', 'Unknown')[:60]}..."
                )
                return article_id
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        article_id = await pipeline.ingest_article(
            url=test_url, keywords_matched=["Epstein", "Wikipedia"]
        )
