...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"{LOG_DIR}/fbi_vault_download_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
...
FBI_URLS = [
    # Main Epstein files
    (
        "https://vault.fbi.gov/Epstein%20Jeffrey/Epstein%20Jeffrey%20Part%2001%20of%2001/view",
        "epstein_part_01.pdf",
    ),
    # Additional files if available
    (
        "https://vault.fbi.gov/Epstein%20Jeffrey%20Jeffrey%20Epstein%20Part%201%20of%202%20/view",
        "epstein_part_1_1.pdf",
    ),
    (
        "https://vault.fbi.gov/Epstein%20Jeffrey%20Jeffrey%20Epstein%20Part%202%20of%202%20/view",
        "epstein_part_1_2.pdf",
    ),
]
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if (
                    ...
                    and os.path.getsize(dest_path) > 1000
                ):
...
def main():
    ...
    logging.info("\n" + "=" * 60)
    logging.info(
        f"FBI VAULT DOWNLOAD COMPLETE: {downloaded} downloaded, {skipped} skipped, {failed} failed"
    )
    logging.info("=" * 60)
