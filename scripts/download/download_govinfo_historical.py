...
def download_collection_range(collection, year, start_date, end_date):
    ...
    if count is not None:
        logger.info(
            f"  Skipping {COLLECTION_NAMES[collection]} {start_date}..{end_date}; existing file has {count} packages"
        )
    ...
    while True:
        ...
        while retry_count < max_retries:
            try:
                ...
                logger.info(
                    f"  {COLLECTION_NAMES[collection]} {year}: Page {page_count}, {len(packages)} packages, total: {len(total_packages)}"
                )
                ...
                if not next_page:
                    ...
                    logger.info(
                        f"✅ Downloaded {len(total_packages)} packages for {COLLECTION_NAMES[collection]} {start_date}..{end_date} to {path}"
                    )
                    return collection, year, len(total_packages)
                ...
                if not offset_mark:
                    raise RuntimeError(
                        f"Missing offsetMark in nextPage for {collection} {start_date}..{end_date}"
                    )
                break
                ...
                if retry_count < max_retries:
                    logger.warning(
                        f"  Retry {retry_count}/{max_retries} for {COLLECTION_NAMES[collection]} {year} at offsetMark {offset_mark}: {e}"
                    )
                    time.sleep(2**retry_count)
                else:
                    logger.error(
                        f"  Failed to download {COLLECTION_NAMES[collection]} {year} at offsetMark {offset_mark} after {max_retries} retries: {e}"
                    )
                    # Save what we have
                    ...
                    logger.info(
                        f"  Saved partial download: {len(total_packages)} packages to {partial_path}"
                    )
                    return collection, year, len(total_packages)
...
def main():
    ...
    parser.add_argument(
        "--years", default="2000-2024", help="Years to download, e.g. '2000' or '2000-2024'"
    )
    parser.add_argument(
        "--collections",
        default=",".join(COLLECTIONS),
        help="Comma-separated GovInfo collection codes",
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(download_collection_year, collection, year): (collection, year)
            for collection, year in tasks
        }
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.info(
                    f"✅ {COLLECTION_NAMES[collection]} {year} completed: {result[2]} packages"
                )
            except Exception as e:
