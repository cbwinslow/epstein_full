...
class ImportMonitor:
    ...
    def check_processes(self) -> Dict[str, bool]:
        ...
        for pid in pids:
            ...
            try:
                cmd = subprocess.run(
                    ["ps", "-p", pid, "-o", "comm,args"], capture_output=True, text=True
                )
    ...
    def get_directory_size(self, dataset: str) -> str:
        ...
        try:
            result = subprocess.run(["du", "-sh", str(dataset_dir)], capture_output=True, text=True)
        ...
        for log_file in sorted(
            ...
            try:
                with open(log_file, "r") as f:
                    ...
                    progress[log_file.name] = (
                        "".join(last_lines[-3:]) if last_lines else "No content"
                    )
            except:
                pass
    ...
    def generate_report(self) -> Dict:
        ...
        report = {
            ...
            "processes": {
                "govinfo_bulk_download": "RUNNING"
                if running.get("govinfo_bulk")
                else "NOT RUNNING",
