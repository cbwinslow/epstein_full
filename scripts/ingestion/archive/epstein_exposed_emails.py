...
def fetch_page(page, per_page=BATCH_SIZE):
    ...
    resp = requests.get(
        API_BASE, params={"page": page, "per_page": per_page}, headers=headers, timeout=30
    )
...
def run_download():
    ...
    if state.get("last_success"):
        ...
        if elapsed < timedelta(minutes=50):
            log(
                f"Last success was {elapsed.total_seconds() / 60:.0f}min ago. Skipping (wait 50min between runs)."
            )
            save_state(state)
            return 0
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            log(
                f"Page {page}: {len(emails)} fetched, {new} new. Total: {state['total_downloaded']}/{total_available}"
            )
            ...
            if "Rate limited" in error_msg:
                log(
                    f"Rate limited on attempt {attempt + 1}/{MAX_RETRIES}. Waiting {RETRY_DELAY}s..."
                )
