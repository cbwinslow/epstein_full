...
def run_phase3_extraction(test_mode=False, limit=10, batch_size=100, rate_limit=2.0):
    ...
    from scripts.ingestion.phase3_extraction import run_bulk_extraction, run_test_sample
...
def run_full_pipeline(args):
    ...
    # Phase 1: Discovery
    discovered_count = run_phase1_discovery(args.keywords, args.start_date, args.end_date)
...
Examples:
  ...
  python run_pipeline.py --phase extraction --test --limit 20
    ...
    parser.add_argument(
        "--phase",
        choices=["discovery", "extraction", "all"],
        default="all",
    ...
    parser.add_argument(
        "--start-date",
        type=lambda s: __import__("datetime").datetime.strptime(s, "%Y-%m-%d").date(),
    ...
    parser.add_argument("--limit", type=int, default=10, help="Number of URLs to test (test mode)")
    parser.add_argument(
        "--batch-size", type=int, default=100, help="Batch size for bulk extraction"
    )
    parser.add_argument(
        "--rate-limit",
        type=float,
    ...
    parser.add_argument("--test", action="store_true", help="Shortcut for --mode test")
