...
def extract_with_fallback(url: str) -> Optional[Dict]:
    ...
    try:
        ...
        return {"title": title, "text": content, "description": description, "url": url}
    ...
    try:
        ...
        """,
            (
                ...
                else json.dumps({"categories": result.categories}),
            ),
        )
    ...
    for batch_num, i in enumerate(range(0, total, batch_size), 1):
        ...
        for queue_id, url, title in batch:
            ...
            if result.success:
                ...
                logger.info(
                    f"  ✓ {result.word_count} words - {result.title[:50] if result.title else 'No title'}..."
                )
            else:
                batch_fail += 1
                logger.error(f"  ✗ Failed - {result.error}")
