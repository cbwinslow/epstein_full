    ...
    def get_stats(self, run_id: uuid.UUID) -> Dict:
        ...
        with psycopg2.connect(self.dsn) as conn:
            with conn.cursor() as cur:
                ...
                row = cur.fetchone()
                return {"total": row[0], "pending": row[1], "completed": row[2], "failed": row[3]}
...
class MegaParallelOrchestrator:
    ...
    async def run_stage(
        self, stage: Dict, discovery_config: DiscoveryConfig, collection_config: CollectionConfig
    ):
        """Run a single ingestion stage."""
        run_id = stage["run_id"]

        logger.info(
            f"Stage {stage['stage_num']}: {stage['start_date'].date()} to {stage['end_date'].date()}"
        )
...
def run_epstein_ingestion(
    start_year: int = 2000, end_year: int = 2025, keywords: List[str] = None, max_stages: int = 30
):
    """
    ...
    collection_config = CollectionConfig(
        batch_size=200, max_concurrent_collectors=20, rate_limit_seconds=0.3
    )
