...
async def create_ingestion_run(
    ...
    config_json = json.dumps(
        {"script_version": "2.0", "keywords": keywords, "date_range": [start_date, end_date]}
    )
...
async def update_run_results(
    run_id: str, discovered: int, queued: int, collected: int, failed: int, sources_breakdown: dict
):
    ...
    logger.info(
        f"[RUN {run_id[:8]}] Updated results: {discovered} discovered, {queued} queued, {collected} collected"
    )


async def complete_ingestion_run(
    run_id: str, status: str = "completed", error_message: Optional[str] = None
):
...
async def discover_articles(keywords, start_date, end_date, max_results, run_id: str):
    ...
    results = await agent.search(
        keywords=keywords, date_range=(start_date, end_date), max_results=max_results
    )
...
async def queue_articles(storage, articles, run_id: str):
    ...
    logger.info(
        f"[RUN {run_id[:8]}] Queued {queued} new, linked {linked} existing (total: {total_queued})"
    )
...
async def collect_single_article(storage, collector, item, run_id, idx, total):
    ...
    try:
        ...
        if result and result.get("stored_id"):
            storage.update_queue_status(item["id"], "completed", result_id=result["stored_id"])
            logger.info(f"  [{idx}/{total}] ✓ Stored: {result.get('title', 'N/A')[:50]}")
            return "success"
        else:
            storage.update_queue_status(
                item["id"], "failed", error_message="Collection returned None"
            )
        ...
        storage.update_queue_status(item["id"], "failed", error_message=str(e)[:500])
        return "error"
...
async def collect_articles(storage, run_id: str, batch_size=100, max_concurrent=5):
    ...
    while True:
        ...
        items = storage.get_queued_items(
            media_type="news", status="pending", limit=batch_size, ingestion_run_id=run_id
        )
        ...
        logger.info(
            f"[Progress] Processed: {total_processed}/{total_pending} | Collected: {collected} | Failed: {failed}"
        )
        logger.info(
            f"Processing batch of {len(items)} articles with {max_concurrent} concurrent workers..."
        )
        ...
        logger.info(
            f"[Batch Complete] Processed: {total_processed}/{total_pending} | Collected: {collected} | Failed: {failed}"
        )
...
async def main():
    ...
    parser.add_argument(
        "--max-results", type=int, default=5000, help="Maximum articles to discover"
    )
    parser.add_argument("--batch-size", type=int, default=50, help="Collection batch size")
    parser.add_argument(
        "--skip-discovery", action="store_true", help="Skip discovery, only collect queued items"
    )
    parser.add_argument(
        "--skip-collection", action="store_true", help="Skip collection, only discover and queue"
    )
    ...
    try:
        ...
        if not args.skip_collection:
            ...
            sources_breakdown = (
                agent.metrics.get("sources_breakdown", {}) if "agent" in locals() else {}
            )
            await update_run_results(
                run_id, len(articles), queued, collected, failed, sources_breakdown
            )
