...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            log_dir / f"media_acquisition_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
    ],
)
...
class MediaAcquisitionRunner:
    ...
    async def initialize(self):
        ...
        self.storage = StorageManager(connection_string=DATABASE_URL, base_path=MEDIA_BASE_PATH)
    ...
    async def collect_news(self, batch_size=10):
        ...
        queue_items = self.storage.get_queued_items(
            media_type="news", status="pending", limit=batch_size
        )
    ...
    async def discover_videos(self, max_results=20):
        ...
        for keyword in EPSTEIN_KEYWORDS[:3]:  # Start with top 3 keywords
            try:
                ...
                videos = await agent.search(keywords=[keyword], max_results=max_results)
    ...
    async def discover_documents(self, max_results=20):
        ...
        for keyword in EPSTEIN_KEYWORDS[:3]:
            try:
                ...
                documents = await agent.search(keywords=[keyword], max_results=max_results)
