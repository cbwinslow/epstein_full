...
from scripts.ingestion.news_ingestion_framework import CollectionQueue, run_epstein_ingestion
...
def run_full_collection():
    ...
    print(
        f"Success rate: {results['total_collected'] / max(results['total_discovered'], 1) * 100:.1f}%"
    )
...
def run_test_collection():
    ...
    results = run_epstein_ingestion(
        start_year=2024, end_year=2024, keywords=["Jeffrey Epstein", "Epstein"], max_stages=5
    )
...
def show_stats():
    ...
    # Total articles
    cur.execute(
        "SELECT COUNT(*) FROM media_news_articles WHERE discovery_source = 'mega_parallel_ingestion'"
    )
    ...
    cur.execute(
        "SELECT COUNT(*), COUNT(CASE WHEN status = 'pending' THEN 1 END) FROM collection_queue"
    )
...
def main():
    ...
    parser.add_argument("--stats", action="store_true", help="Show current collection statistics")
    parser.add_argument("--start", type=int, default=2000, help="Start year (for custom range)")
    parser.add_argument("--end", type=int, default=2025, help="End year (for custom range)")
    parser.add_argument(
        "--max-stages", type=int, default=30, help="Max concurrent stages (default 30)"
    )
