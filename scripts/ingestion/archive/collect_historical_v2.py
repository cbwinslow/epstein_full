...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            "/home/cbwinslow/workspace/epstein-data/logs/historical_collection.log"
        ),
...
TARGET_URLS = {
    ...
    "Epstein Coverage": [
        ...
        (
            "miamiherald.com",
            ["jeffrey-epstein", "perversion-of-justice", "epstein-sex-trafficking"],
        ),
...
class DirectWaybackCollector:
    ...
    async def get_available_snapshots(
        self, original_url: str, from_date: str, to_date: str
    ) -> List[str]:
    ...
    async def collect_from_source(
        ...
        for keyword in keywords:
            ...
            # Get snapshots
            timestamps = await self.get_available_snapshots(search_url, from_date, to_date)
...
async def run_historical_collection(category: str, output_file: str, max_per_source: int = 50):
    ...
    async with DirectWaybackCollector(rate_limit=1.0) as collector:
        for domain, keywords in sources:
            ...
            results = await collector.collect_from_source(
                domain=domain, keywords=keywords, date_range=date_range, max_per_keyword=10
            )
