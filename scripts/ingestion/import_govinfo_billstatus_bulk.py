...
def normalize_bill_payload(root: ET.Element, source_file: str) -> Dict:
    ...
    summary_items = children(summaries_parent, "./summary") or children(summaries_parent, "./billSummaries/item")
    ...
    for item in children(subjects_parent, "./legislativeSubjects/item") + children(subjects_parent, "./policyArea"):
...
def insert_summaries(cur, bill_id: int, bill: ET.Element, source_file: str):
    summaries_parent = bill.find("./summaries")
    items = children(summaries_parent, "./summary") or children(summaries_parent, "./billSummaries/item")
    for item in items:
...
def insert_actions_and_votes(cur, bill_id: int, bill: ET.Element, congress: int, source_file: str):
    for item in children(bill.find("./actions"), "./item"):
        committees = []
        for committee in children(item.find("./committees"), "./item") + children(item, "./committees/committee"):
            committees.append(
                {
        ...
        for link in links:
            ...
            if not url:
                continue
            if any(token in url.lower() for token in ["roll", "vote"]) or "Record Vote Number" in name:
                chamber = "House" if "house" in url.lower() or "clerk.house.gov" in url.lower() else ("Senate" if "senate" in url.lower() else "")
                cur.execute(
                    """
...
def insert_related_bills(cur, bill_id: int, bill: ET.Element, source_file: str):
    for item in children(bill.find("./relatedBills"), "./item"):
        details = []
        for rel in children(item.find("./relationshipDetails"), "./item") + children(item.find("./relationshipDetails"), "./relationshipDetail"):
            details.append(
                {
...
def import_zip(zip_path: Path) -> Dict[str, int]:
    stats = {"bills": 0, "titles": 0, "summaries": 0, "actions": 0, "cosponsors": 0, "related_bills": 0, "vote_refs": 0}
    conn = get_db_connection()
    cur = conn.cursor()
    try:
    ...
    try:
        verify_cur.execute("SELECT count(*) FROM congress_bills WHERE source_file LIKE %s", (f"{zip_path}:%",))
        stats["bills"] = verify_cur.fetchone()[0]
        verify_cur.execute("SELECT count(*) FROM congress_bill_titles WHERE source_file LIKE %s", (f"{zip_path}:%",))
        stats["titles"] = verify_cur.fetchone()[0]
        verify_cur.execute("SELECT count(*) FROM congress_bill_summaries WHERE source_file LIKE %s", (f"{zip_path}:%",))
        stats["summaries"] = verify_cur.fetchone()[0]
        verify_cur.execute("SELECT count(*) FROM congress_bill_actions WHERE source_file LIKE %s", (f"{zip_path}:%",))
        stats["actions"] = verify_cur.fetchone()[0]
        verify_cur.execute("SELECT count(*) FROM congress_bill_cosponsors WHERE source_file LIKE %s", (f"{zip_path}:%",))
        stats["cosponsors"] = verify_cur.fetchone()[0]
        verify_cur.execute("SELECT count(*) FROM congress_bill_related_bills WHERE source_file LIKE %s", (f"{zip_path}:%",))
        stats["related_bills"] = verify_cur.fetchone()[0]
        verify_cur.execute("SELECT count(*) FROM congress_bill_vote_references WHERE source_file LIKE %s", (f"{zip_path}:%",))
...
def main():
    ...
    totals = {"bills": 0, "titles": 0, "summaries": 0, "actions": 0, "cosponsors": 0, "related_bills": 0, "vote_refs": 0}
    failed = []
