    ...
    for year in years:
        ...
        try:
            ...
            if len(r.content) < 5000:
                logger.warning(f"Suspiciously small response for {year}: {len(r.content)} bytes — skipping")
                continue
            ...
            for hit in hits:
                ...
                rows.append({
                    ...
                    "pdf_url": src.get("pdf_url") or (
                        f"https://efts.senate.gov/LATEST/search-index/documents/{report_id}"
                    ),
                })
            if len(rows) >= 500:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                ON CONFLICT (source_name) DO UPDATE SET
                    ...
                    last_updated = NOW()
            """, ("House Financial Disclosures", "government", "house_financial_disclosures", house_count, "complete" if house_count > 0 else "failed"))
            ...
            cur.execute("""
                ...
                ON CONFLICT (source_name) DO UPDATE SET
                    ...
                    status = EXCLUDED.status,
                    last_updated = NOW()
            """, ("Senate Financial Disclosures", "government", "senate_financial_disclosures", senate_count, "complete" if senate_count > 0 else "failed"))
        conn.commit()
...
if __name__ == "__main__":
    ...
    conn.close()
    logger.info("=" * 80)
    logger.info(f"FINANCIAL DISCLOSURES INGESTION COMPLETE: House={house_total}, Senate={senate_total}")
    logger.info("=" * 80)
