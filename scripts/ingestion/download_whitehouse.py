...
def download_file(key: str, url: str) -> Dict:
    ...
    try:
        ...
        if response.status_code == 200 and len(response.content) > 100:
            ...
            logger.warning(f"⚠️ Failed: {key} (status={response.status_code}, size={len(response.content)})")
...
def download_obama():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        futures = {executor.submit(download_zip_file, key, url): key for key, url in OBAMA_VISITOR_LOGS.items()}
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(download_file, key, url): key for key, url in VISITOR_LOGS.items()}
    ...
    return {"success": success, "skipped": skipped, "failed": failed, "total_size_mb": total_size/1024/1024}
