...
def download_bills_for_congress(congress):
    ...
    if existing and all(item.get('congress') == congress for item in existing if isinstance(item, dict)):
        logger.info(f"  Skipping Congress {congress} bills; existing file has {len(existing)} records")
    ...
    while True:
        ...
        while retry_count < max_retries:
            try:
                ...
                logger.info(f"  Congress {congress} bills: Page {page_count}, {len(page_bills)} bills, total: {len(bills)}/{total_bills}")
                ...
                if len(bills) >= total_bills:
                    ...
                    logger.info(f"✅ Downloaded {len(bills)} bills for Congress {congress} to {output_file}")
                    return congress, len(bills)
                ...
                if retry_count < max_retries:
                    logger.warning(f"  Retry {retry_count}/{max_retries} for Congress {congress} bills at offset {offset}: {e}")
                    time.sleep(2 ** retry_count)  # Exponential backoff
                else:
                    logger.error(f"  Failed to download bills for Congress {congress} at offset {offset} after {max_retries} retries: {e}")
                    # Save what we have
                    output_file = congress_dir / f"bills_{congress}_partial.json"
                    with open(output_file, 'w', encoding='utf-8') as f:
                        json.dump({'congress': congress, 'bills': bills, 'partial': True}, f, indent=2)
...
def download_members_for_congress(congress):
    ...
    existing = load_existing_json(output_file)
    if existing:
        logger.info(f"  Skipping Congress {congress} members; existing file has {len(existing)} records")
        return congress, len(existing)
    ...
    while True:
        ...
        while retry_count < max_retries:
            try:
                ...
                logger.info(f"  Congress {congress} members: Page {page_count}, {len(page_members)} members, total: {len(members)}/{total_members}")
                ...
                if len(members) >= total_members:
                    ...
                    logger.info(f"✅ Downloaded {len(members)} members for Congress {congress} to {output_file}")
                    return congress, len(members)
                ...
                retry_count += 1
                if retry_count < max_retries:
                    logger.warning(f"  Retry {retry_count}/{max_retries} for Congress {congress} members at offset {offset}: {e}")
                    time.sleep(2 ** retry_count)  # Exponential backoff
                else:
                    logger.error(f"  Failed to download members for Congress {congress} at offset {offset} after {max_retries} retries: {e}")
                    # Save what we have
                    output_file = congress_dir / f"members_{congress}_partial.json"
                    with open(output_file, 'w', encoding='utf-8') as f:
                        json.dump({'congress': congress, 'members': members, 'partial': True}, f, indent=2)
                    logger.info(f"  Saved partial download: {len(members)} members to {output_file}")
                    return congress, len(members)
...
def download_house_votes_for_congress(congress):
    ...
    existing = load_existing_json(output_file)
    if existing:
        logger.info(f"  Skipping Congress {congress} House votes; existing file has {len(existing)} records")
        return congress, len(existing)
        ...
        while retry_count < max_retries:
            try:
                ...
                if len(votes) >= total_votes:
                    ...
                    logger.info(f"✅ Downloaded {len(votes)} House votes for Congress {congress} to {output_file}")
                    return congress, len(votes)
                ...
                retry_count += 1
                if retry_count < max_retries:
                    logger.warning(f"  Retry {retry_count}/{max_retries} for Congress {congress} House votes at offset {offset}: {e}")
                    time.sleep(2 ** retry_count)
                else:
                    logger.error(f"  Failed to download House votes for Congress {congress} at offset {offset} after {max_retries} retries: {e}")
                    partial_file = congress_dir / f"house_votes_{congress}_partial.json"
                    with open(partial_file, 'w', encoding='utf-8') as f:
                        json.dump({'congress': congress, 'houseRollCallVotes': votes, 'partial': True}, f, indent=2)
                    logger.info(f"  Saved partial download: {len(votes)} House votes to {partial_file}")
                    return congress, len(votes)
...
def import_congress_directory(congress):
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"Import failed for Congress {congress} with exit code {result.returncode}")
...
def main():
    ...
    parser.add_argument('--import-after-download', action='store_true', help='Import each congress immediately after its download finishes')
    parser.add_argument(
        '--components',
    ...
    logger.info(f"Average speed: {(total_bills + total_members + total_house_votes)/duration:.2f} records/second")
