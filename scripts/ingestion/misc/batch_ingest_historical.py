...
from media_acquisition.config import DATABASE_URL, get_storage_manager
...
async def main():
    ...
    async with BatchIngestionManager(batch_size=args.batch_size) as manager:
        results = await manager.run_ingestion(urls_file=args.urls_file, resume=not args.no_resume)
    ...
    print(
        f"Success rate: {results['success'] / results['processed'] * 100:.1f}%"
