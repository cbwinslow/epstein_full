...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/tmp/historical_collection.log"), logging.StreamHandler()],
)
...
class HistoricalOrchestrator:
    ...
    def discover_urls(self, year_start: int, year_end: int, test_mode: bool = False) -> str:
        ...
        # Build command
        cmd = [sys.executable, "scripts/collect_historical_data.py", str(year_start), str(year_end)]
        ...
        result = subprocess.run(
            cmd, capture_output=True, text=True, cwd="/home/cbwinslow/workspace/epstein"
        )
...
def main():
    ...
    parser.add_argument("--test", action="store_true", help="Run in test mode (small batches)")
    ...
    parser.add_argument(
        "--period",
        choices=["all", "911", "mid", "late", "recent", "current"],
        default="all",
