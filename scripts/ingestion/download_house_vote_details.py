        ...
        rows.append(
            {
                ...
                "member_name": legislator.attrib.get("unaccented-name") or legislator.attrib.get("sort-field"),
    ...
    async with semaphore:
        try:
            async with session.get(url, timeout=30) as resp:
                ...
                return congress, session_number, roll_call, expected_vote_date, None, f"HTTP {resp.status}"
...
def import_vote_detail(
    ...
    if vote_date is None:
        vote_date = expected_vote_date[:10] if isinstance(expected_vote_date, str) and len(expected_vote_date) >= 10 else "1900-01-01"
    legislation_type = vote.get("legislationType") or ""
    legislation_number = vote.get("legislationNumber") or ""
    bill_id = f"{legislation_type}{legislation_number}".strip() if (legislation_type or legislation_number) else None
...
async def main():
    ...
    for res in results:
        ...
        try:
            count = import_vote_detail(congress, roll_call, session_number, expected_vote_date, data)
            total_members += count
            successful += 1
