        ...
        try:
            ...
            if e.response.status_code == 429:
                ...
                if retry_count >= max_retries:
                    logger.error(f"Max retries ({max_retries}) exceeded for {endpoint} page {params['page']}")
                    raise
                backoff = min(2 ** retry_count, 60)  # Exponential backoff, max 60s
                logger.warning(f"Rate limited (429). Waiting {backoff}s before retry {retry_count}/{max_retries}")
                time.sleep(backoff)
            else:
                raise
        ...
        with conn.cursor() as cur:
            cur.execute("""
                ...
                ON CONFLICT (source_name) DO UPDATE SET
                    ...
                    last_updated = NOW()
            """, ("Senate LDA Lobbying", "government", "lda_filings", "imported", None, total_records))
        conn.commit()
