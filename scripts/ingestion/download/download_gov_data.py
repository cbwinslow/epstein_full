...
class GovInfoClient:
    ...
    def __init__(self, api_key: str = GOVINFO_API_KEY):
        ...
        self.session.headers.update(
            {"Accept": "application/json", "User-Agent": "EpsteinResearch/1.0"}
        )
    ...
    def search_financial_disclosures(
        self, member_name: str = None, start_date: str = None
    ) -> List[Dict]:
        """
        ...
        try:
            response = self.session.post(
                url, params={"api_key": self.api_key}, json=payload, timeout=60
            )
            response.raise_for_status()
    ...
    def get_collection_updates(
        self, collection_code: str, start_date: str, end_date: str = None
    ) -> List[Dict]:
        ...
        params = {"api_key": self.api_key, "offsetMark": "*", "pageSize": 1000}
...
class CongressGovClient:
    ...
    def search_bills(self, query: str, congress: int = 118) -> List[Dict]:
        ...
        params = {"api_key": self.api_key, "congress": congress, "q": query, "limit": 100}
...
def main():
    ...
    parser.add_argument(
        "--search-disclosures", action="store_true", help="Search financial disclosures"
    )
    ...
    if args.collections:
        ...
        for c in collections:
            ...
            if any(
                x in code.lower() or x in name.lower()
                for x in ["congress", "house", "senate", "court"]
            ):
