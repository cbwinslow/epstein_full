...
print(
    f"Downloaded: {stats.files_downloaded if hasattr(stats, 'files_downloaded') else 'unknown'} files"
)
print()
...
reconstruct(
    input_dir=str(raw_dir), output_dir=str(recon_dir), language="en", processes=4, verbose=True
)
...
filtermerge(input_dir=str(recon_dir), output_file=str(final_csv), query=query, verbose=True)
