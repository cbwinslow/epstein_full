...
def fetch_vote_xml(congress: int, session: int, vote_number: int) -> Tuple[int, int, int, Optional[bytes], Optional[str], str]:
    url = (
...
def upsert_vote_detail(conn, congress: int, session: int, vote_number: int, root: ET.Element, source_url: str):
    cur = conn.cursor()
...
def upsert_member_votes(conn, congress: int, session: int, vote_number: int, root: ET.Element, source_url: str) -> int:
...
def main():
    ...
    parser.add_argument("--congresses", default="117-119", help="Congress selector, e.g. 117-119 or 118,119")
    parser.add_argument("--sessions", default="1,2", help="Session selector, e.g. 1,2")
    parser.add_argument("--concurrency", type=int, default=30, help="Parallel vote-detail fetch workers")
    parser.add_argument("--limit", type=int, default=0, help="Optional cap on vote details to process")
    parser.add_argument("--rate-delay", type=float, default=0.03, help="Minimum delay between HTTP requests")
    parser.add_argument("--max-retries", type=int, default=5, help="HTTP retry attempts for transient failures")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, args.concurrency)) as executor:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            try:
                ...
                success += 1
                member_rows += count
                logger.info("Imported Senate vote %s-%s-%s with %s member rows", congress, session, vote_number, count)
            except Exception as e:  # noqa: BLE001
                conn.rollback()
                failed += 1
