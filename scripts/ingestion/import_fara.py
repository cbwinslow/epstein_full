...
def create_tables():
    ...
    """)
    cur.execute("ALTER TABLE fara_registrations ADD COLUMN IF NOT EXISTS foreign_principal_state TEXT;")
    cur.execute("ALTER TABLE fara_registrations ADD COLUMN IF NOT EXISTS foreign_principal_zip TEXT;")
    cur.execute("ALTER TABLE fara_registrations ADD COLUMN IF NOT EXISTS raw_data JSONB;")
    cur.execute("ALTER TABLE fara_registrations ADD COLUMN IF NOT EXISTS imported_at TIMESTAMPTZ DEFAULT NOW();")
...
def import_foreign_principals(records: List[Dict]) -> int:
    records = _dedupe_rows(
        records, ["registration_number", "principal_name", "foreign_principal", "principal_country", "country_location_represented"]
    )
    if not records:
        return 0
    ...
    for r in records:
        values.append((
            ...
            str(r.get('foreign_political_party', '')).strip().lower() in {'true', '1', 'yes', 'y'},
            json.dumps(r),
            r.get('source_file')
        ))
...
def import_short_forms(records: List[Dict]) -> int:
    records = _dedupe_rows(
        records, ["registration_number", "short_form_last_name", "short_form_first_name", "short_form_date"]
    )
    if not records:
        return 0
    conn = get_db_connection()
...
def main():
    ...
    parser.add_argument("--base-dir", default=str(BASE_DIR), help="Directory containing FARA XML files")
    ...
    for xml_file in xml_files:
        ...
        logger.info(
            ...
            totals["registrations"], totals["principals"], totals["short_forms"], totals["docs"]
        )
    ...
    logger.info(
        ...
        totals["registrations"], totals["principals"], totals["short_forms"], totals["docs"], grand_total
    )
    logger.info("=" * 80)
