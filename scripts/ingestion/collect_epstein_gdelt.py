...
def download_gdelt_range(start_date: str, end_date: str) -> bool:
    ...
    try:
        ...
        download(start_date, end_date, outdir=str(outdir), decompress=True, show_progress=True)
...
def filter_epstein_articles(input_dir: str, output_file: str) -> bool:
    ...
    try:
        logger.info("Filtering for Epstein content")
        filtermerge(input_dir=input_dir, output_file=output_file, query=EPSTEIN_QUERY, verbose=True)
