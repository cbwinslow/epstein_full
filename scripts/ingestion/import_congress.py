    ...
    try:
        ...
        for member in members_list:
            ...
            record = {
                ...
                'depiction_image_url': member.get('depiction', {}).get('imageUrl', '') if member.get('depiction') else '',
...
def parse_bills_file(filepath: Path) -> List[Dict]:
    ...
    try:
        ...
        for bill in payload_records(data, 'bills'):
            ...
            committees = [c.get('name', '') for c in bill.get('committees', []) if isinstance(c, dict)]
            ...
            records.append(record)
        if skipped:
            logger.warning(f"Skipped {skipped} bills in {filepath.name} that did not match congress {expected_congress}")
        if skipped_invalid:
        ...
        try:
            cur.execute("""
                ...
                ON CONFLICT (identifier) DO UPDATE SET
                    ...
                    imported_at = NOW()
            """, (
                r['identifier'], r['congress'], r['session_number'], r['roll_call_number'],
                r['vote_type'], r['result'], r['legislation_type'], r['legislation_number'],
                r['legislation_url'], r['amendment_type'], r['amendment_number'],
                r['amendment_author'], r['source_data_url'], r['api_url'], r['start_date'],
                r['update_date'], r['raw_data'], r['source_file']
            ))
            inserted += 1
        except Exception as e:
...
def main():
    ...
    parser.add_argument('--base-dir', type=Path, default=BASE_DIR, help='Directory containing Congress JSON files')
    ...
    logger.info("=" * 80)
    logger.info(f"IMPORT COMPLETE: {total_members} members, {total_bills} bills, {total_house_votes} House votes")
    logger.info("=" * 80)
