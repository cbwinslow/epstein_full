...
async def create_tables(conn):
    ...
    """)
    await conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_phones_entry_id ON black_book_phones(contact_entry_id)"
    )
    ...
    """)
    await conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_emails_entry_id ON black_book_emails(contact_entry_id)"
    )
    ...
    """)
    await conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_addresses_entry_id ON black_book_addresses(contact_entry_id)"
    )
...
async def import_data(conn):
    ...
    for page_num, page_data in pages.items():
        ...
        for contact in contacts:
            ...
            for phone in contact.get("phones", []):
                phones_batch.append(
                    (entry_id, phone.get("type"), phone.get("number"), phone.get("formatting"))
                )
    ...
    if emails_batch:
        await conn.copy_records_to_table(
            "black_book_emails", records=emails_batch, columns=["contact_entry_id", "email"]
        )

    if addresses_batch:
        await conn.copy_records_to_table(
            "black_book_addresses", records=addresses_batch, columns=["contact_entry_id", "address"]
        )
...
async def verify(conn):
    ...
    for s in samples:
        print(
            f"   - {s['full_name']} (Section {s['alphabetical_section']}, Page {s['page_number']})"
        )
