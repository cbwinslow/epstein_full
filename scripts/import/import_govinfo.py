...
def infer_collection(filepath: Path, data: Dict, pkg: Dict) -> tuple[str, str]:
    ...
    collection_name = pkg.get("collectionName") or collection_names.get(
...
def parse_packages_file(filepath: Path) -> List[Dict]:
    ...
    try:
        ...
        for pkg in payload_packages(data):
            ...
            collection_code, collection_name = infer_collection(
                filepath, data if isinstance(data, dict) else {}, pkg
            )
            ...
            record = {
                ...
                "download_pdf_url": pkg.get("download", {}).get("pdfLink", "")
                if pkg.get("download")
                else "",
                "download_txt_url": pkg.get("download", {}).get("txtLink", "")
                if pkg.get("download")
                else "",
                "download_xml_url": pkg.get("download", {}).get("xmlLink", "")
                if pkg.get("download")
                else "",
        ...
        try:
            ...
            """,
                (
                    ...
                    raw.get("abstract") or "",
                    raw.get("dates") or "",
                    ", ".join(raw.get("agencies", []))
                    if isinstance(raw.get("agencies"), list)
                    else (raw.get("agencies") or ""),
                    raw.get("action") or "",
                    bool(raw.get("significant", False)),
                    ", ".join(raw.get("rindicators", []))
                    if isinstance(raw.get("rindicators"), list)
                    else (raw.get("rindicators") or ""),
                    r["download_pdf_url"],
                    ...
                    raw.get("pageEnd"),
                    raw.get("pages"),
                    ", ".join(raw.get("docketIds", []))
                    if isinstance(raw.get("docketIds"), list)
                    else (raw.get("docketIds") or ""),
                    ", ".join(raw.get("regulationsDotGovIds", []))
                    if isinstance(raw.get("regulationsDotGovIds"), list)
                    else (raw.get("regulationsDotGovIds") or ""),
        ...
        try:
            ...
            """,
                (
                    ...
                    ", ".join(raw.get("judges", []))
                    if isinstance(raw.get("judges"), list)
                    else (raw.get("judges") or ""),
                    ", ".join(raw.get("attorneys", []))
                    if isinstance(raw.get("attorneys"), list)
                    else (raw.get("attorneys") or ""),
                    ", ".join(raw.get("parties", []))
                    if isinstance(raw.get("parties"), list)
                    else (raw.get("parties") or ""),
                    ...
                    ", ".join(raw.get("caseSourceSubdivisions", []))
                    if isinstance(raw.get("caseSourceSubdivisions"), list)
                    else (raw.get("caseSourceSubdivisions") or ""),
                    r["download_pdf_url"],
                    raw.get("packageLink") or r["detail_view_txt_url"],
                    raw.get("summary") or "",
                    raw.get("outcome") or "",
...
def main():
    ...
    parser.add_argument(
        "--base-dir", type=Path, default=BASE_DIR, help="Directory containing GovInfo JSON files"
    )
    ...
    if args.recursive:
        json_files = sorted(
            path for path in args.base_dir.glob("**/*.json") if "_partial" not in path.name
        )
    else:
        json_files = []
        for subdir in ["bills", "crpt", "fr", "uscode", "uscourts"]:
            json_files.extend(
                sorted(
                    path
                    for path in (args.base_dir / subdir).glob("*.json")
