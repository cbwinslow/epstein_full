                ...
                for tag_name in ("RULE", "PRORULE", "NOTICE"):
                    for idx, article in enumerate(root.findall(f".//{tag_name}"), start=1):
                        ...
                        title = (
                            ...
                            or content_meta.get("title", "")
                            or content_meta.get("section_subject", "")
                        )
                        abstract = (
                            clean_text(extract_plain_text(preamb.find("./SUM")))
                            if preamb is not None
                            else ""
                        )
                        dates = (
                            clean_text(extract_plain_text(preamb.find("./EFFDATE")))
                            if preamb is not None
                            else ""
                        ) or (
                            clean_text(extract_plain_text(preamb.find("./DATES")))
                            if preamb is not None
                            else ""
                        )
                        agencies = (
                            ...
                            or (
                                clean_text(extract_plain_text(preamb.find("./AGY")))
                                if preamb is not None
                                else ""
                            )
                            or content_meta.get("agency", "")
                        )
                        action = (
                            clean_text(extract_plain_text(preamb.find("./ACT")))
                            if preamb is not None
                            else ""
                        )
                        rindicators = (
                            clean_text(preamb.findtext("./RIN")) if preamb is not None else ""
                        )
                        prtpage = preamb.find("./PRTPAGE") if preamb is not None else None
                        page_start = (
                            int(re.sub(r"\D", "", prtpage.get("P", "")) or 0) or None
                            if prtpage is not None
                            else None
                        )
                        page_end, number_of_pages = None, None
                        _, parsed_end, parsed_pages = parse_page_range(
                            content_meta.get("pages", "")
                        )
                        ...
                        correction_match = re.search(
                            r"Correction to FR Doc\.?\s+([0-9-]+)",
                            extract_plain_text(article),
                            re.IGNORECASE,
                        )
                        if correction_match:
