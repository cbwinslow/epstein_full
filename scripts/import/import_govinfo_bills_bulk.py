...
def parse_from_member_name(member: str):
    ...
    match = re.match(
        r"bills-(\d+)(hr|s|hjres|sjres|hconres|sconres|hres|sres)(\d+)([a-z0-9]+)?$", stem
    )
    if not match:
        return None
...
def get_or_create_bill(
    cur, congress: int, bill_type: str, bill_number: str, title: str, source_file: str
) -> int:
    cur.execute(
...
def import_zip(zip_path: Path) -> Dict[str, int]:
    ...
    try:
        with zipfile.ZipFile(zip_path) as archive:
            for member in archive.namelist():
                ...
                current_chamber = (
                    clean_text(form.findtext("./current-chamber")) if form is not None else ""
                )
                legis_type = clean_text(form.findtext("./legis-type")) if form is not None else ""
                official_title = (
                    clean_text(form.findtext("./official-title")) if form is not None else ""
                )
                ...
                action_desc = (
                    extract_plain_text(form.find("./action/action-desc"))
                    if form is not None and form.find("./action/action-desc") is not None
                    else ""
                )
                ...
                if metadata is not None:
                    for elem in metadata.iter():
                        ...
                        if (
                            elem.text
                            and tag.startswith("dc:") is False
                            and tag not in {"metadata", "dublinCore"}
                        ):
                ...
                congress = (
                    int(re.sub(r"\D", "", congresst))
                    if re.sub(r"\D", "", congresst)
                    else (fallback[0] if fallback else None)
                )
                if match:
                ...
                cur.execute(
                    ...
                    (
                        ...
                        legis_type,
                        current_chamber,
                        action_date_node.get("date")[:4]
                        + "-"
                        + action_date_node.get("date")[4:6]
                        + "-"
                        + action_date_node.get("date")[6:8]
                        if action_date_node is not None and action_date_node.get("date")
                        else None,
                        action_desc,
                        official_title,
                        short_title,
                        ...
                        extract_plain_text(root.find("./legis-body"))
                        if root.find("./legis-body") is not None
                        else "",
    ...
    try:
        verify_cur.execute(
            "SELECT count(*) FROM congress_bill_text_versions WHERE source_file LIKE %s",
            (f"{zip_path}:%",),
        )
