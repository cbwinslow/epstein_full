...
PARQUET_FILE = Path(
    "/home/cbwinslow/workspace/epstein-data/hf-emails-threads/epstein_email_threads.parquet"
)
...
async def check_duplicates(conn, df):
    ...
    if sample_ids:
        overlap = await conn.fetchval(
            "SELECT COUNT(*) FROM house_oversight_emails WHERE thread_id = ANY($1)", sample_ids
        )
...
async def import_parquet(conn):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            thread_id = (
                str(row.get("thread_id", ""))[:100] if pd.notna(row.get("thread_id")) else None
            )
