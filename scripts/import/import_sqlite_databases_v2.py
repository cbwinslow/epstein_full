...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"{LOG_DIR}/sqlite_import_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ),
...
def import_sqlite_to_postgres(sqlite_path, pg_conn):
    ...
    logging.info(
        f"  Tables: {len(tables)} ({', '.join(tables[:5])}{'...' if len(tables) > 5 else ''})"
    )
...
def main():
    ...
    # Find all .db files
    db_files = [
        os.path.join(databases_dir, f) for f in os.listdir(databases_dir) if f.endswith(".db")
    ]
    db_files.sort()
