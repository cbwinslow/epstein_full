...
async def main():
    ...
    for fs_dir, sql_table in mapping.items():
        if fs_dir in hf_dirs:
            ...
            if isinstance(sql_table, list):
                ...
                total_sql = sum(
                    sql_counts.get(t, 0) for t in sql_table if isinstance(sql_counts.get(t), int)
                )
                ...
                if isinstance(sql_count, int):
                    if sql_count > 0:
                        print(
                            f"   ✅ {fs_dir}: {fs_info['files']} files -> {sql_count:,} records in {sql_table}"
                        )
                    else:
                        print(
                            f"   🔄 {fs_dir}: {fs_info['files']} files -> NOT YET IMPORTED to {sql_table}"
                        )
                else:
    ...
    pending = sum(
        1
        for t in ["hf_embeddings", "hf_epstein_data_text"]
        if isinstance(sql_counts.get(t), int) and sql_counts.get(t, 0) == 0
    )
    ...
    print("   Dropped (duplicates): 1 (hf_email_threads)")
    ...
    if (
        isinstance(sql_counts.get("hf_epstein_data_text"), int)
        and sql_counts.get("hf_epstein_data_text", 0) == 0
    ):
