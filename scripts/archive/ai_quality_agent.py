...
def run_data_quality_check() -> dict:
    ...
    try:
        result = subprocess.run(
            ["python3", validator_path, "--ai-report"], capture_output=True, text=True, timeout=300
        )
...
def save_memory_to_letta(report: dict, summary: str):
    ...
    try:
        ...
        letta = LettaIntegration(server_url="http://localhost:8283", agent_name="windsurf")
...
"""
        ...
        if agent_id:
            letta.save_to_archival(
                text=memory_content, tags=["data-quality", "validation", "epstein-project"]
            )
...
def main():
    parser = argparse.ArgumentParser(description="AI Agent Data Quality Monitor")
    parser.add_argument(
        "--run-check", action="store_true", help="Run data quality check and report"
    )
    parser.add_argument("--analyze", action="store_true", help="Generate AI analysis summary")
    parser.add_argument("--monitor", action="store_true", help="Run continuous monitoring")
    parser.add_argument(
        "--interval", type=int, default=60, help="Monitoring interval in minutes (default: 60)"
    )
    parser.add_argument("--save-memory", action="store_true", help="Save results to Letta memory")
