...
class EpsteinProcessor:
    ...
    def process_dataset(self):
        ...
        logger.info(
            f"Max pages per document: {self.max_pages_per_doc if self.max_pages_per_doc else 'ALL'}"
        )
        logger.info("-" * 50)
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            futures = [
                executor.submit(self.process_single_file, filename) for filename in pdf_files
            ]
            ...
            for i, future in enumerate(as_completed(futures)):
                ...
                if result:
                    ...
                    logger.info(
                        f"{i + 1}/{len(pdf_files)} {result['filename']}: {result['char_count']:,} chars, {len(result['entities'])} entities ({result['processing_time']:.2f}s)"
                    )
                else:
    ...
    def process_single_file(self, filename):
        ...
        text, char_count, page_count, entities = process_pdf_full_pages(
            file_path, self.nlp, self.max_pages_per_doc
        )
...
def process_dataset_full_pages(
    input_dir, output_dir, workers=8, max_files=None, max_pages_per_doc=None
):
