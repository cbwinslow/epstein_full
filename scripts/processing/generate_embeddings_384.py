...
def get_embedding_stats(conn):
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        stats["embedding_coverage_percent"] = (
            round(stats["pages_with_embeddings"] * 100.0 / stats["total_pages"], 2)
...
def generate_embeddings(batch_size, limit=None):
    ...
    try:
        ...
        # Process in batches
        total_to_process = (
            min(limit, stats["pages_without_embeddings"])
            if limit
            else stats["pages_without_embeddings"]
        )
        ...
        while processed < total_to_process:
            ...
            print(
                f"\r  Progress: {processed:,}/{total_to_process:,} "
