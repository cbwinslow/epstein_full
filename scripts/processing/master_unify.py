...
class ProgressTracker:
    ...
    def __init__(self, log_file: Optional[str] = None):
        ...
        self.log_file = (
            log_file or f"{LOG_DIR}/unify_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        self._setup_logging()
    ...
    def complete_operation(self, name: str, message: str = ""):
        ...
        if name in self.operations:
            ...
            rate = current / elapsed if elapsed > 0 else 0
            self.logger.info(
                f"[COMPLETE] {name}: {current} items in {elapsed:.1f}s ({rate:.1f}/sec) - {message}"
            )
...
class DataUnifier:
    ...
    def import_json_to_staging(self, file_path: str, table_name: str, source_name: str) -> int:
        ...
        self.tracker.start_operation(
            f"import_{source_name}", details=f"{file_path} -> {table_name}"
        )
        ...
        try:
            ...
            self.tracker.complete_operation(
                f"import_{source_name}", f"Imported {imported}/{total} records"
            )
            return imported
    ...
    def process_parquet_directory(self, path: str, table_name: str) -> int:
        ...
        self.tracker.start_operation(
            "parquet_import", total=len(files), details=f"Processing {len(files)} parquet files"
        )
        ...
        for i, file_path in enumerate(files):
            try:
                ...
                self.tracker.update_progress(
                    "parquet_import", i + 1, f"{file_path}: {records_imported} rows"
                )
...
def main():
    ...
    if args.background:
        ...
        with open(log_file, "w") as f:
            subprocess.Popen(cmd, stdout=f, stderr=subprocess.STDOUT, cwd=PROJECT_ROOT)
