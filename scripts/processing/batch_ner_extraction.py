...
REGISTRY_PATH = Path(
    "/home/cbwinslow/workspace/epstein/Epstein-Pipeline/data/persons-registry.json"
)
...
def get_documents_from_db(
    conn, batch_size: int = 1000, offset: int = 0, limit: Optional[int] = None
) -> List[Dict]:
...
def run_extract_entities(input_dir: Path, output_dir: Path, registry_path: Path) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info("extract-entities completed successfully")
            if result.stdout:
...
def process_batch(
    conn, batch_num: int, batch_size: int, processed_ids: Set[str], resume: bool
) -> Dict:
    ...
    if success:
        ...
        return {"status": "success", "files": files_written, "output_files": len(output_files)}
    else:
...
def main():
    ...
    parser.add_argument("--limit", type=int, help="Limit total number of documents to process")
    parser.add_argument(
        "--resume", action="store_true", help="Resume from existing output (skip already processed)"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    try:
        ...
        for batch_num in range(num_batches):
            ...
            result = process_batch(conn, batch_num, args.batch_size, processed_ids, args.resume)
