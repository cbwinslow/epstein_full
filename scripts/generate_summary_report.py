...
def generate_report():
    ...
    for row in cur.fetchall():
        print(
            f"{row[0]:<15} {row[1]:<12} {row[2]:>3} sources  {row[4]:>12,} / {row[3]:>12,} ({row[5]:>5.1f}%)"
        )
    ...
    print("-" * 70)
    cur.execute(
        "SELECT filename, status, rows_imported, total_rows FROM icij_import_progress ORDER BY status, filename"
    )
    for row in cur.fetchall():
        filename, status, imported, total = row
        icon = {"complete": "✅", "running": "🔄", "pending": "⏳", "failed": "❌"}.get(
            status, "❓"
        )
        if total and total > 0:
            pct = 100.0 * (imported or 0) / total
            print(
                f"{icon} {filename:<25} {status:<10} {imported:>10,} / {total:>10,} ({pct:>5.1f}%)"
            )
        else:
    ...
    print("-" * 70)
    cur.execute(
        "SELECT source_name, target_table, actual_records, status FROM data_inventory WHERE source_type = 'huggingface' AND status = 'complete' ORDER BY actual_records DESC"
    )
    for row in cur.fetchall():
