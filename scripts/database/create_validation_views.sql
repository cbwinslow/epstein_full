-- ============================================================================
-- EPSTEIN FILES DATABASE VALIDATION VIEWS
...
SELECT 
    ...
    'Pages referencing non-existent documents' as description
FROM pages p
...
SELECT 
    ...
    'missing_document',
    COUNT(*),
    'Entities referencing non-existent documents'
...
SELECT 
    ...
    SUM(LENGTH(content_markdown)) as total_content_length,
    AVG(LENGTH(content_markdown)) as avg_content_length
FROM jmail_emails_full
WHERE sender IS NOT NULL
GROUP BY sender
...
-- ----------------------------------------------------------------------------
-- VIEW 8: Flight-JMail Cross-Reference
-- Validates flight passengers against email participants
-- ----------------------------------------------------------------------------
...
COMMENT ON VIEW v_flight_email_crossref IS 'Cross-reference between flight passengers and email participants';

-- ----------------------------------------------------------------------------
-- VIEW 9: Document Coverage Analysis
...
-- ----------------------------------------------------------------------------
-- VIEW 10: Entity Co-occurrence Network
...
GROUP BY de1.entity_value, de1.entity_type, de2.entity_value, de2.entity_type
