...
class StorageManager:
    ...
    def get_queue_summary(self, media_type: Optional[str] = None) -> Dict[str, Any]:
        """
...
class VideoDiscoveryAgent(DiscoveryAgent):
    ...
    async def search(
        ...
        if self.youtube_enabled:
            youtube_results = await self._search_youtube(keywords, date_range, max_results // 2)
            results.extend(youtube_results)

        # Search Archive.org
        archive_results = await self._search_archive(keywords, date_range, max_results // 2)
...
class TestStorageManager:
    ...
    def test_get_queue_summary(self, storage_manager: StorageManager, clean_database):
        ...
        for i in range(3):
            storage_manager.add_to_queue(
                media_type="news", source_url=f"https://example.com/article{i}", priority=5
            )
    ...
    def test_duplicate_url_handling(self, storage_manager: StorageManager, clean_database):
        ...
        # First add
        id1 = storage_manager.add_to_queue(media_type="news", source_url=url, priority=5)

        # Second add (should return None due to ON CONFLICT DO NOTHING)
        id2 = storage_manager.add_to_queue(media_type="news", source_url=url, priority=5)
...
class TestAgentConfig:
    ...
    def test_custom_config(self):
        ...
        config = AgentConfig(agent_id="test-agent", config={"api_key": "secret123"})
...
class TestNewsDiscoveryAgent:
    ...
    async def test_gdelt_search(self, mock_get, agent_config: AgentConfig, mock_gdelt_response):
        ...
        results = await agent.search(
            keywords=["Epstein"], date_range=("2024-01-01", "2024-01-31"), max_results=10
        )
...
async def test_full_collection_pipeline(
    news_discovery_agent, news_collector, storage_manager, clean_database
):
    """
    ...
    discovered = await news_discovery_agent.search(keywords=keywords, max_results=5)
