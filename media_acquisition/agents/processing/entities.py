...
from media_acquisition.base import AgentConfig, ProcessingAgent, StorageManager, TaskResult
...
class EntityExtractor(ProcessingAgent):
    ...
    async def process_unprocessed_media(self, batch_size: int = 100) -> List[Dict[str, Any]]:
        ...
        for article in articles:
            try:
                result = await self.process_news_article(article["id"], article["content"])
    ...
    async def execute(self, task: Dict[str, Any]) -> TaskResult:
        ...
        if not content:
            return TaskResult(status="failure", error="No content provided")
        ...
        try:
            if media_type == "news":
                ...
                return TaskResult(status="failure", error=f"Unknown media type: {media_type}")

            return TaskResult(status="success", output=result, metrics=self.metrics)

        except Exception as e:
            return TaskResult(status="failure", error=str(e), retry_allowed=True)
