...
class YouTubeTranscriber:
    ...
    def check_captions(self, url: str) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                ["yt-dlp", "--list-subs", url], capture_output=True, text=True, timeout=30
            )
            ...
            for line in lines:
                ...
                if "Available subtitles" in line:
                    ...
                    if len(parts) >= 2:
                        ...
                        captions[current_section].append({"code": lang_code, "name": lang_name})
    ...
    def download_captions(
        self, url: str, video_id: str, language: str = "en", prefer_manual: bool = True
    ) -> Optional[Path]:
        """
        ...
        cmd = [
            ...
            "vtt/best",
            "-o",
            str(self.caption_path / f"{video_id}.%(ext)s"),
            url,
        ]
    ...
    def download_audio(
        self, url: str, video_id: str, format: str = "mp3", quality: str = "best"
    ) -> Optional[Path]:
        """
        ...
        cmd = [
            ...
            quality,
            "--output",
            str(self.audio_path / f"{video_id}.%(ext)s"),
            url,
        ]
    ...
    def transcribe_with_whisper(
        self, audio_path: Path, video_id: str, model: str = None, language: str = "en"
    ) -> Dict[str, Any]:
        """
    ...
    def parse_caption_file(self, caption_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for match in re.finditer(cue_pattern, content, re.DOTALL):
                ...
                segments.append({"start": parse_ts(start), "end": parse_ts(end), "text": text})
    ...
    def transcribe(self, url: str, strategy: str = "auto", language: str = "en") -> Dict[str, Any]:
        """
        ...
        result = {"video_id": video_id, "url": url, "strategy": strategy, "success": False}
        ...
        if strategy in ["whisper", "auto"]:
            ...
            if audio_path:
                # Transcribe
                transcript = self.transcribe_with_whisper(audio_path, video_id, language=language)
                    ...
                    result.update(
                        {
                            ...
                            "transcript_file": str(
                                self.transcript_path / f"{video_id}_whisper.json"
                            ),
                        }
                    )
                    return result
...
class VideoTranscriber(CollectionAgent):
    ...
    def __init__(
        self, config: Optional[AgentConfig] = None, storage: Optional[StorageManager] = None
    ):
    ...
    async def collect(self, video: VideoMetadata, strategy: str = "auto") -> Dict[str, Any]:
        ...
        result = await asyncio.get_event_loop().run_in_executor(
            None, lambda: self.transcriber.transcribe(url, strategy=strategy)
        )
    ...
    async def process_queue(
        self, batch_size: int = 10, strategy: str = "auto"
    ) -> List[Dict[str, Any]]:
        """
        ...
        items = self.storage.get_queued_items(
            media_type="video", status="pending", limit=batch_size
        )
            ...
            try:
                ...
                if result.get("success"):
                    self.storage.update_queue_status(
                        item["id"], "completed", result_id=result.get("stored_id")
                    )
                else:
                    self.storage.update_queue_status(
                        item["id"], "failed", error_message=result.get("error", "Unknown error")
                    )
    ...
    async def execute(self, task: Dict[str, Any]) -> TaskResult:
        ...
        if not videos:
            return TaskResult(status="failure", error="No videos provided")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="base", help="Whisper model (tiny/base/small/medium/large-v3)"
    )
    parser.add_argument("--no-gpu", action="store_true", help="Disable GPU (use CPU)")
    ...
    # Create transcriber
    transcriber = YouTubeTranscriber(whisper_model=args.model, use_gpu=not args.no_gpu)
