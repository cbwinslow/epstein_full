...
class ArticleDownloader:
    ...
    def __init__(
        self, storage_path: str = "/home/cbwinslow/workspace/epstein-data/media/articles/"
    ):
    ...
    def download_article(
        self, article_url: NewsArticleURL, use_wayback: bool = True
        ...
        if (
            not result
            and use_wayback
            and article_url.metadata
            and article_url.metadata.get("wayback_url")
        ):
    ...
    def _download_with_newspaper(
        self, url: str, article_url: NewsArticleURL, is_wayback: bool = False
    ) -> Optional[Dict[str, Any]]:
    ...
    def _download_with_requests(
        self, url: str, article_url: NewsArticleURL
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Downloaded (fallback): {result['title'][:60]} ({result['word_count']} words)"
            )
            return result
...
class NewsCollector(CollectionAgent):
    ...
    def __init__(
        ...
        # Initialize metrics
        self.metrics = {"items_collected": 0, "errors": 0, "sources_breakdown": {}}
    ...
    async def process_article(self, article: NewsArticleURL) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"success": False, "url": article.url, "error": str(e)}
    ...
    async def collect(self, article: NewsArticleURL) -> Optional[Dict[str, Any]]:
        ...
        article_data = await asyncio.get_event_loop().run_in_executor(
            None, lambda: self.downloader.download_article(article, use_wayback=True)
        )
        ...
        if self.storage:
            try:
                ...
                logger.debug(
                    "article_data keys: %s", list(article_data.keys()) if article_data else None
                )
                logger.debug(
                    "article_data url: %s", article_data.get("url") if article_data else None
                )
                ...
                article_dict = {
                    ...
                    "related_person_ids": [],
                    "collection_method": "newspaper3k"
                    if article_data.get("extraction_method") == "newspaper3k"
                    else "requests",
                }
    ...
    async def process_queue(self, batch_size: int = 50) -> List[Dict[str, Any]]:
        ...
        # Get pending items
        items = self.storage.get_queued_items(media_type="news", status="pending", limit=batch_size)
            ...
            try:
                ...
                if result and result.get("stored_id"):
                    self.storage.update_queue_status(
                        item["id"], "completed", result_id=result["stored_id"]
                    )
                    results.append(result)
                else:
                    self.storage.update_queue_status(
                        item["id"], "failed", error_message="Failed to collect article"
                    )
                    self.metrics["errors"] += 1
                ...
                self.storage.update_queue_status(item["id"], "failed", error_message=str(e)[:500])
                self.metrics["errors"] += 1
    ...
    async def execute(self, task: Dict[str, Any]) -> TaskResult:
        ...
        if not articles:
            return TaskResult(status="failure", error="No articles provided")
...
if __name__ == "__main__":
    ...
    # Create article
    article = NewsArticleURL(
        url=args.url, metadata={"wayback_url": args.wayback_url} if args.wayback_url else {}
    )
