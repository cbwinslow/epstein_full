...
class GdeltClient:
    ...
    def query_events(
        self, keywords: List[str], date_range: Tuple[str, str], max_records: int = 250
    ) -> pd.DataFrame:
        """
        ...
        while current <= end:
            ...
            try:
                events = g.Search([date_str], table="events", output="df", coverage=False)
                ...
                if events is not None and len(events) > 0:
                    ...
                    mask = (
                        ...
                        | events["Actor1CountryCode"].str.contains(
                            keyword_str, case=False, na=False
                        )
                        | events["Actor2CountryCode"].str.contains(
                            keyword_str, case=False, na=False
                        )
                    )
...
class WaybackSearcher:
    ...
    def find_snapshots(
        self, domains: List[str], keywords: List[str], date_range: Tuple[str, str]
    ) -> List[NewsArticleURL]:
        """
        ...
        for domain in domains:
            try:
                domain_results = self._search_domain(domain, keywords, date_range)
    ...
    def _search_domain(
        self, domain: str, keywords: List[str], date_range: Tuple[str, str]
    ) -> List[NewsArticleURL]:
        ...
        try:
            response = self.session.get(self.CDX_API, params=params, timeout=60)
            response.raise_for_status()
...
class RSSCrawler:
    ...
    def discover(
        self, feeds: List[str], keywords: List[str], date_range: Tuple[str, str]
    ) -> List[NewsArticleURL]:
        """
                    ...
                    if any(k in title or k in summary for k in keyword_set):
                        ...
                        results.append(
                            NewsArticleURL(
                                ...
                                priority=2,
                                keywords_matched=[
                                    k for k in keywords if k in title or k in summary
                                ],
                                metadata={
                                    "rss_feed": feed_url,
...
class NewsDiscoveryAgent(DiscoveryAgent):
    ...
    DEFAULT_KEYWORDS = [
        ...
        "sex offender Epstein",
        "non-prosecution agreement Epstein",
        "Epstein plea",
        "civil lawsuit Epstein",
        ...
        "Epstein jail death",
        "Epstein documentary",
        "Epstein investigation aftermath",
        # Financial
        "Epstein finances",
        "Epstein money laundering",
        "Epstein tax evasion",
        "Epstein offshore accounts",
        # International
        "Epstein France",
        "Epstein UK",
        "Epstein Caribbean",
    ...
    async def search(
        ...
        **kwargs,
        ...
        if end_date.year >= datetime.now().year - 1:
            ...
            try:
                rss_articles = await self._search_rss(keywords, date_range)
    ...
    async def _search_gdelt(
        self, keywords: List[str], date_range: Tuple[str, str], max_records: int
    ) -> List[NewsArticleURL]:
        ...
        articles = await loop.run_in_executor(
            None, lambda: self.gdelt.query_gkg(keywords, date_range, max_records)
        )
    ...
    async def _search_google_news(
        self, keywords: List[str], date_range: Tuple[str, str], max_results: int
    ) -> List[NewsArticleURL]:
        """Search Google News for articles."""
        loop = asyncio.get_event_loop()

        result = await loop.run_in_executor(
            None, lambda: self.google_news.search(keywords, date_range, max_results)
        )
    ...
    async def _search_wayback(
        self, sources: List[str], keywords: List[str], date_range: Tuple[str, str]
    ) -> List[NewsArticleURL]:
        """Search Wayback Machine for articles."""
        loop = asyncio.get_event_loop()

        results = await loop.run_in_executor(
            None, lambda: self.wayback.find_snapshots(sources, keywords, date_range)
        )

        return results

    async def _search_rss(
        self, keywords: List[str], date_range: Tuple[str, str]
    ) -> List[NewsArticleURL]:
        ...
        results = await loop.run_in_executor(
            None, lambda: self.rss.discover(feeds, keywords, date_range)
        )
    ...
    async def execute(self, task: Dict[str, Any]) -> TaskResult:
        ...
        try:
            results = await self.search(
                keywords=keywords, date_range=date_range, sources=sources, max_results=max_results
            )

            return TaskResult(status="success", output=results, metrics=self.metrics)
