...
from media_acquisition.base import AgentConfig, DiscoveryAgent, DocumentMetadata, TaskResult
...
class CourtListenerClient:
    ...
    def search_cases(
        ...
        params = {"q": query, "order_by": "dateFiled desc", "page_size": min(max_results, 100)}

        if date_filed_after:
        # ``cases`` holds a list of raw case dictionaries returned from the API.
        # Explicit type annotation silences mypy's ``var-annotated`` warning.
        ...
        try:
        # ``params`` is passed directly to ``requests.get`` which expects a mapping
        # of ``str`` to ``str``/``int``/``float``/``Iterable``.  The concrete type is
        # not important for runtime behaviour, so we silence the ``arg-type`` error.
        params = {"q": query, "order_by": "dateFiled desc", "page_size": min(max_results, 100)}  # type: ignore[arg-type]
    ...
    def search_dockets(
        ...
        params = {"q": query, "order_by": "-date_last_filing", "page_size": min(max_results, 100)}
...
class DocumentDiscoveryAgent(DiscoveryAgent):
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        self.govinfo = GovInfoClient(api_key=getattr(config, "govinfo_api_key", None))
            ...
            try:
                ...
                for result in results:
                    doc = DocumentMetadata(
                        ...
                        metadata={"court": result.get("court"), "snippet": result.get("snippet")},
                    )
    ...
    async def search(
        ...
        **kwargs,
        ...
        if "courtlistener" in sources:
            ...
            try:
                cl_results = await self._search_courtlistener(keywords, date_range, max_results)
        ...
        if "govinfo" in sources:
            ...
            try:
                gov_results = await self._search_govinfo(keywords, date_range, max_results)
        ...
        params = {"query": query, "pageSize": min(max_results, 100)}  # type: ignore[arg-type]
        return deduplicated
