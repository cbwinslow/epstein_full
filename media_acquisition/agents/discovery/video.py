...
from media_acquisition.base import AgentConfig, DiscoveryAgent, TaskResult, VideoMetadata
            ...
            for doc in data.get("response", {}).get("docs", []):
                ...
                videos.append(
                    VideoMetadata(
                        ...
                        metadata={"subjects": doc.get("subject", []), "identifier": identifier},
                    )
                )
...
class VideoDiscoveryAgent(DiscoveryAgent):
    ...
    def _initialize_resources(self):
        ...
        logger.info(
            f"VideoDiscoveryAgent initialized (YouTube API: {'enabled' if self.youtube.api_key else 'disabled'})"
        )
    ...
    async def search(
        ...
        **kwargs,
        ...
        if "internet_archive" in platforms:
            ...
            try:
                ia_results = await self._search_internet_archive(keywords, date_range, max_results)
        ...
        for video in all_videos:
            if video.platform == "youtube":
                ...
                transcript_info = self.internet_archive.check_transcript_availability(
                    video.video_id
                )
                video.transcript_available = transcript_info.get("available", False)
                video.transcript_source = transcript_info.get("source")
