...
class BaseAgent(ABC):
    ...
    def _log_error(self, error: Exception, context: Dict = None):
        ...
        self.logger.error(
            f"Agent {self.AGENT_ID} error: {error}",
            exc_info=True,
...
class StorageManager:
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    RETURNING id
                """, (media_type, source_url, priority,
                      keywords_matched or [], discovered_by, ingestion_run_id))
    ...
    def get_queued_items(self,
                        ...
                        status: str = 'pending',
                        limit: int = 100,
                        ingestion_run_id: str = None) -> List[Dict]:
