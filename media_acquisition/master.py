...
class MediaAcquisitionSystem:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.storage = StorageManager(
            connection_string=self.config.postgres_url, base_path=self.config.storage_path
        )
        ...
        # State
        self.running = False
        self.stats = {"started_at": None, "items_discovered": 0, "items_collected": 0, "errors": 0}
        ...
        for media_type in media_types:
            ...
            try:
                ...
                await self._run_discovery_phase(
                    media_type=media_type, keywords=keywords, date_range=(start_date, end_date)
                )
    ...
    async def _run_discovery_phase(self, media_type: str, keywords: List[str], date_range: tuple):
        ...
        results = await agent.search(keywords=keywords, date_range=date_range)
                ...
                try:
                    ...
                    if result:
                        self.storage.update_queue_status(
                            item["id"], "completed", result_id=result.get("id")
                        )
                        collected += 1
                    else:
                        self.storage.update_queue_status(
    ...
    async def _run_processing_phase(self, media_type: str):
        ...
        logger.info("[Processing] Phase complete (placeholder)")
...
Examples:
  ...
  python -m media_acquisition.master --mode historical --keywords Epstein Maxwell --start-date 2023-01-01
    ...
    parser.add_argument(
        "--mode",
        choices=["historical", "monitor", "discover-only"],
        default="historical",
        help="Operation mode",
    )
    ...
    parser.add_argument(
        "--keywords", nargs="+", default=["Epstein", "Maxwell"], help="Search keywords"
    )
    ...
    if args.mode == "historical":
        ...
        asyncio.run(system.run_continuous_monitoring(interval_hours=args.interval))
